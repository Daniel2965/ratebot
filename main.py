...
async def cmd_start(message: types.Message, state: FSMContext):
    # Обработка реферальной ссылки (например, /start ref_123456)
...
async def set_search_gender_cb(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(f"✅ Настройки поиска сохранены! Вы будете оценивать: **{val_map[choice]}**", parse_mode="Markdown")
    await callback.answer()
...
async def start_rating(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        search_gender = user.get('search_gender', 'opposite')
